...
void SchurVINS::InitImuModel(double acc_n, double acc_w, double gyr_n,
                             double gyr_w) {
  imu_noise = Matrix12d::Zero();
  // LOG(INFO) << "acc_n: " << acc_n << ", acc_w: " << acc_w << ", gyr_n: " <<
  // gyr_n << ", gyr_w: " << gyr_w;
  imu_noise.block<3, 3>(0, 0) =
      Eigen::Matrix3d::Identity() * gyr_n * gyr_n;  // acc_n * acc_n;
  imu_noise.block<3, 3>(3, 3) =
      Eigen::Matrix3d::Identity() * acc_n * acc_n;  // acc_w * acc_w;
  imu_noise.block<3, 3>(6, 6) =
      Eigen::Matrix3d::Identity() * acc_w * acc_w;  // gyr_n * gyr_n;
  imu_noise.block<3, 3>(9, 9) =
      Eigen::Matrix3d::Identity() * gyr_w * gyr_w;  // gyr_w * gyr_w;
}
...
void SchurVINS::AugmentState(const svo::FrameBundle::Ptr frame_bundle) {
  ...
  svo::AugStatePtr aug_ptr(new svo::AugState(curr_state->ts, curr_state->id,
                                             curr_state->quat, curr_state->pos,
...
void SchurVINS::PredictionState(const Eigen::Vector3d& acc,
                                const Eigen::Vector3d& gyro, double dt) {
  const Eigen::Vector3d wd_full = gyro * dt;
  const Eigen::Vector3d wd_half = wd_full * 0.5;
  Eigen::Quaterniond dq_half =
      Eigen::Quaterniond(1, wd_half(0) / 2, wd_half(1) / 2, wd_half(2) / 2);
  Eigen::Quaterniond dq_full =
      Eigen::Quaterniond(1, wd_full(0) / 2, wd_full(1) / 2, wd_full(2) / 2);
  dq_half.normalize();
  dq_full.normalize();
...
void SchurVINS::Prediction(double _dt, const Eigen::Vector3d& _acc,
                           const Eigen::Vector3d& _gyr) {
  ...
  Matrix15d Phi =
      Matrix15d::Identity() + Fdt + 0.5 * Fdt_square + (1.0 / 6.0) * Fdt_cube;
  ...
  // fej impl
  // {
  //     Eigen::Matrix3d rr = (curr_state->quat *
  //     curr_state->quat_fej.inverse()).toRotationMatrix(); Eigen::Matrix3d yy
  //     = - Utility::SkewMatrix(curr_state->pos - curr_state->pos_fej -
  //     curr_state->vel_fej * _dt
  //     - 0.5 * gravity * _dt * _dt); Eigen::Matrix3d ss = -
  //     Utility::SkewMatrix(curr_state->vel - curr_state->vel_fej - gravity *
  //     _dt); Phi.block<3, 3>(0, 0) = rr; Phi.block<3, 3>(3, 0) = yy;
  //     Phi.block<3, 3>(6, 0) = ss;
  // }

  cov.block(0, 0, 15, 15) =
      Phi * cov.block(0, 0, 15, 15) * Phi.transpose() +
      Phi * G * imu_noise * G.transpose() * Phi.transpose() * _dt;

  if (!states_map.empty()) {
    int cov_len = cov.rows();
    cov.block(0, 15, 15, cov_len - 15) =
        Phi * cov.block(0, 15, 15, cov_len - 15);
    cov.block(15, 0, cov_len - 15, 15) =
        cov.block(15, 0, cov_len - 15, 15) * Phi.transpose();
  }
...
void SchurVINS::Solve3() {
  ...
  const int min_frame_idx =
      states_map.begin()->second->frame_bundle->getBundleId();
  const int prev_frame_id0 =
                (++states_map.rbegin())->second->frame_bundle->frames_[0]->id(),
            prev_frame_id1 =
                (++states_map.rbegin())->second->frame_bundle->frames_[1]->id();
  const int max_frame_idx =
      states_map.rbegin()->second->frame_bundle->getBundleId();
  ...
  schur_pts_.clear();
  for (svo::LocalPointMap::iterator pit = local_pts.begin();
       pit != local_pts.end(); ++pit) {
    ...
    if (curr_pt->CheckStatus() == false ||
        curr_pt->CheckLocalStatus() == false ||
        curr_pt->CheckLocalStatus(prev_frame_id0, prev_frame_id1,
                                  max_frame_idx) == false) {
    ...
    for (svo::LocalFeatureMap::iterator fit = curr_pt->local_obs_.begin();
         fit != curr_pt->local_obs_.end(); ++fit) {
      ...
      if (curr_frame_idx < min_frame_idx ||
          curr_frame_idx > max_frame_idx)  // pass observations on schurvins
                                           // local sliding window
        continue;

      if (feature->state.expired()) continue;

      const svo::AugStatePtr& ft_state = feature->state.lock();
      const int& state_idx = ft_state->index;
      const svo::Transformation& T_imu_cam =
          feature->camera_id == 0 ? T_imu_cam0 : T_imu_cam1;
      ...
      const Eigen::Matrix3d R_c_w =
          (ft_state->quat * q_i_c).toRotationMatrix().transpose();
      const Eigen::Vector3d Pi =
          ft_state->quat.inverse() * (Pw - ft_state->pos);
      ...
      if (svo::LocalFeature::unit_sphere) {
        r = feature->tan_space * (feature->xyz - Pc.normalized()) *
            (focal_length / level_scale);
      } else {
        r = (feature->xyz.head<2>() - Pc.head<2>() / Pc.z()) *
            (focal_length / level_scale);
      }
      total_error += r.norm();
      ...
      if (r_l2 > huberB) {
        ...
        double rho1 =
            std::max(std::numeric_limits<double>::min(), huberA / radius);
      ...
      if (svo::LocalFeature::unit_sphere) {
        ...
        Eigen::Matrix3d norm_jacob = Eigen::Matrix3d::Identity() * inv_norm -
                                     Pc * Pc.transpose() * inv_norm3;
        dr_dpc = feature->tan_space * norm_jacob *
                 (focal_length * obs_invdev * huber_scale / level_scale);
      } else {
      ...
      dpc_dpos.leftCols(3).noalias() =
          R_i_c.transpose() * Utility::SkewMatrix(Pi) *
          ft_state->quat.toRotationMatrix().transpose();
      dpc_dpos.rightCols(3) = -R_c_w;

      // dpc_dext.leftCols(3).noalias() = Utility::SkewMatrix(Pc) *
      // R_i_c.transpose(); dpc_dext.rightCols(3) = -R_i_c.transpose();
      ...
      ////////////////////////////////////////////////////////////
      // Amtx.block(0, 0, 6, 6).noalias() += jext.transpose() * jext;  // ext 2
      // ext Matrix6d blk_ext2pos = jext.transpose() * jx; const int pos_bias =
      // 6 + ft->state->index * 6;
      ...
      Amtx.block(pos_bias, pos_bias, 6, 6).noalias() +=
          jx.transpose() * jx;  // pos 2 pos
  ...
  for (const svo::PointPtr& pt : schur_pts_) {
    ...
    for (svo::LocalFeatureMap::iterator iti = pt->local_obs_.begin();
         iti != pt->local_obs_.end(); iti++) {
      if (iti->second->state.expired()) continue;
      const svo::AugStatePtr& ft_state = iti->second->state.lock();
      svo::StateFactorMap::iterator iter =
          pt->state_factors.find(ft_state->index);
      if (iter == pt->state_factors.end()) {
        pt->state_factors.emplace(ft_state->index,
                                  svo::StateFactor(iti->second->W, ft_state));
      } else {
  ...
  for (const svo::PointPtr& pt : schur_pts_) {
    ...
    for (svo::StateFactorMap::iterator iti = pt->state_factors.begin();
         iti != pt->state_factors.end(); iti++) {
      if (iti->second.state.expired()) continue;
      const svo::AugStatePtr& statei =
          iti->second.state.lock();  // int pi_bias = 6 + statei->index * 6;
      const int pi_bias = statei->index * 6;

      // const Matrix6d e2p_schur = eWVinv * fti->W.transpose();  // ext 2 pos
      // schur Amtx.block(0, pi_bias, 6, 6) -= e2p_schur;               //
      ...
      for (svo::StateFactorMap::iterator itj = iti;
           itj != pt->state_factors.end(); itj++) {
        if (itj->second.state.expired()) continue;
        const svo::AugStatePtr& statej = itj->second.state.lock();
        // int pj_bias = 6 + statej->index * 6;
        const int pj_bias = statej->index * 6;

        Matrix6d p2p_schur =
            WVinv * itj->second.W.transpose();  // posi 2 posj schur
  ...
  // LOG(ERROR) << "total local pts: " << local_pts.size() << " schur pts: " <<
  // schur_pts_.size();
}

void SchurVINS::StateUpdate(const Eigen::MatrixXd& hessian,
  ...
  Eigen::MatrixXd S =
      hessian * cov.bottomRightCorner(rows, rows) * hessian.transpose() + R;
  Eigen::MatrixXd K_T = S.ldlt().solve(hessian * cov.bottomRows(rows));
  Eigen::MatrixXd K = K_T.transpose();
  // std::cout << "K: " << S.rows() << " " << S.cols() << " " << hessian.rows()
  // << " " << hessian.cols() << std::endl; std::cout << "K: " << cov.rows() <<
  // " " << cov.cols() << " " <<cov.bottomRows(rows).rows() << " " <<
...
void SchurVINS::StateCorrection(const Eigen::MatrixXd& K,
  ...
  const Eigen::Quaterniond dq_imu = Eigen::Quaterniond(
      1.0, 0.5 * dx_imu[0], 0.5 * dx_imu[1], 0.5 * dx_imu[2]);
  ...
  for (svo::StateMap::value_type& item : states_map) {
    ...
    Eigen::Quaterniond dq =
        Eigen::Quaterniond(1.0, 0.5 * dAX[0], 0.5 * dAX[1], 0.5 * dAX[2]);
    dq.normalize();
...
void SchurVINS::Management(int index) {
  ...
  while (src_idx < state_max && dst_idx < state_max) {
    ...
    if (src_idx != dst_idx) {
      cov.block(dst_idx * 6 + 15, 0, 6, cov_len) =
          cov.block(src_idx * 6 + 15, 0, 6, cov_len);
      cov.block(0, dst_idx * 6 + 15, cov_len, 6) =
          cov.block(0, src_idx * 6 + 15, cov_len, 6);
    }
    src_idx++;
    dst_idx++;
  }
  ...
  CHECK(((int)states_map.size() * 6 + 15) == cov.rows())
      << states_map.size() << ", " << cov.rows();
  ManageLocalMap();
}

void SchurVINS::ManageLocalMap() {
  for (svo::LocalPointMap::iterator it = local_pts.begin();
       it != local_pts.end();) {
...
bool SchurVINS::KeyframeSelect() {
  ...
  CHECK((int)states_map.size() == state_max)
      << "states_map.size: " << states_map.size()
      << ", state_max: " << state_max;
  ...
  size_t rindex = 0;
  int common_cnt = 0;
  for (svo::StateMap::reverse_iterator rit = states_map.rbegin();
       rit != states_map.rend();) {
    if (1 == rindex) {
  ...
  // LOG(ERROR) << "delete_r2th: " << delete_r2th << ", track_rate: " <<
  // track_rate
  ...
  for (const svo::FramePtr& frame : frame_bundle->frames_) {
    ...
    for (size_t i = 0; i < frame->numFeatures(); ++i) {
      ...
      if (point != nullptr && svo::isCorner(frame->type_vec_[i])) {
        const svo::LocalFeaturePtr ft = std::make_shared<svo::LocalFeature>(
            state_ptr, point, frame, frame->f_vec_.col(i), camera_id);
    ...
    // LOG(INFO) << "RegisterPoints camera id: " << camera_id << " num feature:
    // " << num_feature;
  }
}

void SchurVINS::InitState(double _ts, const Eigen::Vector3d& _acc,
                          const Eigen::Vector3d& _gyr) {
  if (curr_state->ts > 0) return;

  Eigen::Vector3d acc_norm = _acc / _acc.norm();
  Eigen::Vector3d grav_norm = Eigen::Vector3d(0, 0, 1);
  // LOG(INFO) << "acc: " << _acc[0] << ", " << _acc[1] << ", " << _acc[2] << ",
  // gyr: " << _gyr[0] << ", " << _gyr[1]
  //           << ", " << _gyr[2];

  Eigen::Quaterniond quat =
      Eigen::Quaterniond::FromTwoVectors(acc_norm, grav_norm);
  curr_state->ts = _ts;
  ...
  // LOG(INFO) << "InitState: " << curr_state->ts << ", quat: " <<
  // curr_state->quat.w() << ", " << curr_state->quat.x()
  //           << ", " << curr_state->quat.y() << ", " << curr_state->quat.z()
  //           << ", "
  //           << "pos: " << curr_state->pos[0] << ", " << curr_state->pos[1] <<
  //           ", " << curr_state->pos[2] << ", "
  //           << "vel: " << curr_state->vel[0] << ", " << curr_state->vel[1] <<
  //           ", " << curr_state->vel[2] << ", "
  //           << "ba: " << curr_state->ba[0] << ", " << curr_state->ba[1] << ",
  //           " << curr_state->ba[2] << ", "
  //           << "bg: " << curr_state->bg[0] << ", " << curr_state->bg[1] << ",
  //           " << curr_state->bg[2];

  curr_state->quat_fej = curr_state->quat;
  curr_state->pos_fej = curr_state->pos;
  curr_state->vel_fej = curr_state->vel;
  // Eigen::Quaterniond quat2 = Eigen::Quaterniond::FromTwoVectors(grav_norm,
  // acc_norm); std::cout << "quat2.rot: " << std::endl <<
  // quat2.toRotationMatrix() << std::endl;
}
...
void SchurVINS::Forward(const svo::FrameBundle::Ptr frame_bundle) {
  ...
  for (size_t i = 0; i < frame_bundle->imu_datas_.size(); i++) {
    ...
    const Eigen::Vector3d tmp_acc =
        frame_bundle->imu_datas_[i].linear_acceleration_;
    const Eigen::Vector3d tmp_gyr =
        frame_bundle->imu_datas_[i].angular_velocity_;
  ...
  // update result
  svo::Transformation T_world_imu =
      svo::Transformation(curr_state->quat, curr_state->pos);
  for (const svo::FramePtr& frame : frame_bundle->frames_) {
...

  {
    ...
    // LOG(INFO) << "schurvins forward: " << std::fixed << std::setprecision(6)
    // << curr_state->ts << ", quat: " << quat.w()
...
int SchurVINS::Backward(const svo::FrameBundle::Ptr frame_bundle) {
  ...
  int idx = 0;
  std::for_each(
      states_map.begin(), states_map.end(),
  ...
  for (const svo::StateMap::value_type& item : states_map) {
    const svo::Transformation T_world_imu =
        svo::Transformation(item.second->quat, item.second->pos);
  ...
  if (states_map.size() == 1) {
    ...
    num_valid_feature =
        RemoveOutliers(frame_bundle);  // remove curr frame outlier first
...

  {
    ...
    Eigen::Vector3d ba = curr_state->ba;
    Eigen::Vector3d bg = curr_state->bg;
    // LOG(INFO) << "schurvins backward: " << std::fixed << std::setprecision(6)
    // << curr_state->ts
    //           << ", quat: " << quat.w() << ", " << quat.x() << ", " <<
    //           quat.y() << ", " << quat.z() << ", "
    //           << "pos: " << pos[0] << ", " << pos[1] << ", " << pos[2] << ",
...
bool SchurVINS::StructureOptimize(const svo::PointPtr& optimize_point) {
  if (schur_pts_.count(optimize_point)) {
    ...
    const int max_idx =
        states_map.rbegin()->second->frame_bundle->getBundleId();
    optimize_point->EkfUpdate(dsw_, obs_dev, focal_length, min_idx, max_idx,
                              huberA, huberB);
  ...
  //            << " key obs: " << optimize_point->obs_.size() << " pos: " <<
  //            optimize_point->pos().norm()
...
int SchurVINS::RemovePointOutliers() {
  ...
  for (svo::LocalPointMap::iterator pit = local_pts.begin();
    ...
    for (svo::LocalFeatureMap::iterator fit = curr_pt->local_obs_.begin();
      ...
      // const svo::AugStatePtr& ft_state = feature->state.lock();
      // const int state_idx = ft_state->index;
      // const svo::Transformation& T_imu_cam = feature->camera_id == 0 ?
      // T_imu_cam0 : T_imu_cam1; const Eigen::Matrix3d R_i_c =
      // T_imu_cam.getRotationMatrix(); const Eigen::Quaterniond q_i_c =
      // T_imu_cam.getEigenQuaternion(); const Eigen::Vector3d t_i_c =
      // T_imu_cam.getPosition(); const Eigen::Matrix3d R_c_w = (ft_state->quat
      // * q_i_c).toRotationMatrix().transpose(); const Eigen::Vector3d Pw =
      // curr_pt->pos(); const Eigen::Vector3d Pi = ft_state->quat.inverse() *
      // (Pw - ft_state->pos);
      ...
      const Eigen::Vector2d r =
          (feature->xyz.head<2>() - Pc.head<2>() / Pc.z()) *
          (focal_length / level_scale);
      total_error += r.norm();
      ++num_obs;
    }
...
int SchurVINS::RemoveOutliers(const svo::FrameBundle::Ptr frame_bundle) {
  ...
  for (const svo::FramePtr& frame : frame_bundle->frames_) {
    ...
    // const Eigen::Matrix3d R_c_w = (state->quat *
    // q_i_c).toRotationMatrix().transpose();
    ...
    for (size_t i = 0; i < frame->numFeatures(); ++i) {
      ...
      if (curr_pt && local_pts.find(curr_pt->id()) != local_pts.end()) {
        if (curr_pt->CheckStatus() == false) continue;

        const Eigen::Vector3d Pc = frame->T_f_w_ * curr_pt->pos();
        const Eigen::Vector2d r =
            (obs.head<2>() / obs.z() - Pc.head<2>() / Pc.z()) *
            (focal_length / level_scale);
        total_error += r.norm();
  ...
  // LOG(INFO) << "RemoveOutliers total error: " << total_error << "
  // num_total_feature: " << state->features.size()
  //           << " num_valid_feature: " << num_valid_feature << "
  //           num_invalid_feature: " << num_invalid_feature;
